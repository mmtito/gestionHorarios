...
import { OptPrint } from './models/optPrint';
...
submitExcel(){
  ...
  printSchedule(opt : OptPrint) {

    const querys = `?${opt.entity}=${opt.id}&fd=${opt.start}&td=${opt.end}`;

    return this.http.get<Blob>(`${this.initialUrlSchedule}/generate-report${querys}`, 
    {
       observe: 'response', responseType: 'blob' as 'json'
